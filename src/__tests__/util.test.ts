...
1234 meow !@#`;
        ...
        expect(
            getSecretFromSharesStr([
                '1:2t4v:8vo:16lm:17wg:1ege:1702:17x5:1egc',
                '2:2t4v:97c:16hj:17z7:1eq5:17af:180l:1eq1',
            ]),
        ).toBe('⭐🗝️🗝️');
    });
    ...
    it('getSecretFromSharesStr decodes shares generated by the makeSharesStr fn if prime is sufficiently large', () => {
        ...
        expect(getSecretFromSharesStr(makeSharesStr(secret, 10, prime))).toBe(
            secret,
        );
    });

    it('getSecretFromSharesStr decodes shares generated by the makeSharesStr fn if prime is sufficiently large and secret contains emoji', () => {
        const prime = 2 ** 17 - 1;
        const secret = '🗝️⭐⚔️';
        const shares = makeSharesStr(secret, 4, prime);
        expect(shares.length).toBe(4);
